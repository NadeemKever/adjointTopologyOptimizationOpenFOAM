...
void Foam::adjointOutletVelocityPowerFvPatchVectorField::updateCoeffs()
{
	...
	scalarField Up_ns = phip/patch().magSf(); // mag of normal of primal vel

	vectorField Up_t = Up - (phip*patch().Sf())/( patch().magSf() * patch().magSf()); //tangential velocity

	vectorField Uaneigh = Uap.patchInternalField();  //neighboring adjoint vel

	vectorField Uaneigh_n = (Uaneigh & patch().nf()) *patch().nf(); //normal of the adjoint neighbor

	vectorField Uaneigh_t = Uaneigh - Uaneigh_n;	//tangent of the adjoint neigh
