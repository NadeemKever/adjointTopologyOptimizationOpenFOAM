...
void Foam::adjointOutletPressurePowerFvPatchScalarField::updateCoeffs()
{
	...
	scalarField Up_n = phip / patch().magSf(); // Primal velocity mag

	scalarField Uap_n = phiap / patch().magSf(); //Adjoint velocity mag
	...
	operator==( (Up_n*Uap_n) + (Uap&Up) + nueff*(Uap_n - Uaneigh_n) * deltainv -
